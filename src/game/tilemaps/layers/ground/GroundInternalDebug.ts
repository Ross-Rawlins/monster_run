...
interface GroundInternalCacheEntry {
  ...
  resolvedOffsetsByRequest: Map<number, number>
}
...
function getOrCreateGroundInternalCache(
  ...
  const created: GroundInternalCacheEntry = {
    ...
    resolvedOffsetsByRequest: new Map<number, number>(),
  }
  groundInternalCache.set(tiles, created)
  return created
}

function hasSingleColumnInternalSpan(mask: boolean[][]): boolean {
  for (let row = 0; row < mask.length; row += 1) {
    let col = 0
    while (col < mask[row].length) {
      if (!mask[row][col]) {
        col += 1
        continue
      }
      ...
      let runLength = 0
      while (col < mask[row].length && mask[row][col]) {
        runLength += 1
        col += 1
      }
...
function resolveEffectiveOffset(
  ...
  const cached = cacheEntry.resolvedOffsetsByRequest.get(requestedOffset)
  if (cached !== undefined) {
    return cached
  }
...
export function isGroundInternalDebugCell(
  ...
  const effectiveOffset = resolveEffectiveOffset(
    tiles,
    cacheEntry,
    normalizedOffset
  )
  const qualifiedMask = getQualifiedMaskForOffset(
    tiles,
    cacheEntry,
    effectiveOffset
  )
